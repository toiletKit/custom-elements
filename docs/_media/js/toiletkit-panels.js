class JoomlaPanelsElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Is this nested
    if (this.findAncestorByTagNme(this, 'toiletkit-tab')) {
      this.isNested = true;
    }

    // Does it have child tab element
    if (this.querySelector('toiletkit-tab')) {
      this.hasNested = true;
    }
  ...
  createNavigation() {
    ...
    this.panels.forEach((panel) => {
      if (!panel.id) {
        throw new Error('`toiletkit-panels` All panels require an ID');
      }
  ...
  setTabState() {
    ...
    if (this.hasNested) {
      ...
      if (this.tabLinkHash.length && this.tabLinkHash[0] !== '') {
        ...
        if (element) {
          const currentTabSet = this.findAncestorByTagNme(element, 'toiletkit-tab');
          const parentTabSet = this.findAncestorByTagNme(currentTabSet, 'toiletkit-tab');
...
customElements.define('toiletkit-panels', JoomlaPanelsElement);
