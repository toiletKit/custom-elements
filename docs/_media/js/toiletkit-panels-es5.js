  ...
  function b() {
    _classCallCheck(this, b);var a = _possibleConstructorReturn(this, (b.__proto__ || Object.getPrototypeOf(b)).call(this));return a.hasActive = !1, a.currentActive = '', a.hasNested = !1, a.isNested = !1, a.tabs = [], a.tabsLinks = [], a.panels = [], a.tabLinkHash = [], a;
      ...
      var a = this,
        ...
        if (!d.id) throw new Error('`toiletkit-panels` All panels require an ID');if (d.parentNode === a) {
      ...
      if (this.currentActive) {
        var a = this.querySelector('a[aria-controls="' + this.currentActive + '"]');this.dispatchCustomEvent('toiletkit.tab.hide', a, this.querySelector('#tab-' + this.currentActive)), a.removeAttribute('active'), a.setAttribute('tabindex', '-1'), this.querySelector('#' + this.currentActive).removeAttribute('active'), this.querySelector('#' + this.currentActive).setAttribute('aria-hidden', 'true'), a.removeAttribute('aria-selected'), this.dispatchCustomEvent('toiletkit.tab.hidden', a, this.querySelector('#tab-' + this.currentActive));
      }
    } }, { key: 'activateTabFromLink', value: function activateTabFromLink(a) {
      a.preventDefault();var b = this.currentActive;this.hasActive && this.hideCurrent(), this.dispatchCustomEvent('toiletkit.tab.show', a.target, this.querySelector('#tab-' + b)), a.target.setAttribute('active', ''), a.target.setAttribute('aria-selected', 'true'), a.target.setAttribute('tabindex', '0'), this.querySelector(a.target.hash).setAttribute('active', ''), this.querySelector(a.target.hash).removeAttribute('aria-hidden'), this.currentActive = a.target.hash.substring(1), this.dispatchCustomEvent('toiletkit.tab.shown', a.target, this.querySelector('#tab-' + b)), this.saveState('#tab-' + a.target.hash.substring(1));
      ...
      var a = this,
        ...
        if (this.tabLinkHash.length && '' !== this.tabLinkHash[0]) {
          var d = this.tabLinkHash[0].substring(5),
            ...
            var f = this.findAncestorByTagNme(e, 'toiletkit-tab'),
                g = this.findAncestorByTagNme(f, 'toiletkit-tab');if (g) {
      ...
      var a = window.location.href.match(/#\S[^&]*/),
        ...
        var c = this.findAncestorByTagNme(b, 'toiletkit-tab'),
            d = this.findAncestorByTagNme(c, 'toiletkit-tab');if (d) {
...
}(HTMLElement);customElements.define('toiletkit-panels', JoomlaPanelsElement);
