...
var _createClass = function () {
  ...
  function b() {
    _classCallCheck(this, b);var a = _possibleConstructorReturn(this, (b.__proto__ || Object.getPrototypeOf(b)).call(this));return a.modal = a, a.triggerBtn = '', a.focusableElements = null, a.focusableSelectors = ['a[href]', 'area[href]', 'input:not([disabled])', 'select:not([disabled])', 'textarea:not([disabled])', 'button:not([disabled])', 'iframe', 'object', 'embed', '[contenteditable]', '[tabindex]:not([tabindex^="-"])'], a.width = '', a.height = '', a.innerWidth = '', a.innerHeight = '', a.iframe = '', a.container = a.querySelector('.toiletkit-modal-dialog'), a.title = a.getAttribute('title') || 'Modal', a;
      ...
      if (!this.id) throw new Error('`Joomla-modal` requires an id');if (this.setAttribute('role', 'dialog'), this.classList.add('fade'), this.iframe = this.getAttribute('iframe') || '', this.width = this.getAttribute('width') || '100%', this.height = this.getAttribute('height') || '600px', !this.container) {
        var b = document.createElement('div');b.classList.add('toiletkit-modal-dialog'), b.setAttribute('role', 'document'), b.innerHTML = this.innerHTML, this.innerHTML = '', this.appendChild(b), this.container = this.querySelector('.toiletkit-modal-dialog');
      ...
      var a = this,
        ...
        b.findAncestorByClass(c.target, 'toiletkit-modal-dialog') || c.target === a.triggerBtn || b.close();
...
}(HTMLElement);customElements.define('toiletkit-modal', JoomlaModalElement);
