class JoomlaModalElement extends HTMLElement {
  constructor() {
    ...
    this.innerHeight = '';
    this.iframe = '';
    this.container = this.querySelector('.toiletkit-modal-dialog');
  ...
  connectedCallback() {
    ...
    if (!this.container) {
      ...
      cont.classList.add('toiletkit-modal-dialog');
  ...
  open() {
    ...
    document.addEventListener('click', (event) => {
      if (!self.findAncestorByClass(event.target, 'toiletkit-modal-dialog') && event.target !== this.triggerBtn) {
        self.close();
      }
    });
...
customElements.define('toiletkit-modal', JoomlaModalElement);
