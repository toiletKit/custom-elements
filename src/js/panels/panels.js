;(() => {
  ...
  class TkPanelsElement extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Is this nested
      if (this.findAncestorByTagNme(this, 'tk-tab')) {
        this.isNested = true;
      }

      // Does it have child tab element
      if (this.querySelector('tk-tab')) {
        this.hasNested = true;
      }
    ...
    createNavigation() {
      ...
      this.panels.forEach((panel) => {
        if (!panel.id) {
          throw new Error('`tk-panels` All panels require an ID');
        }
    ...
    setTabState() {
      ...
      if (this.hasNested) {
        ...
        if (this.tabLinkHash.length && this.tabLinkHash[0] !== '') {
          ...
          if (element) {
            const currentTabSet = this.findAncestorByTagNme(element, 'tk-tab');
            const parentTabSet = this.findAncestorByTagNme(currentTabSet, 'tk-tab');
