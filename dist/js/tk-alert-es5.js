...
var _createClass = function () {
  ...
  function b() {
    return _classCallCheck(this, b), _possibleConstructorReturn(this, (b.__proto__ || Object.getPrototypeOf(b)).apply(this, arguments));
      ...
      this.dispatchCustomEvent('tk.alert.show'), this.setAttribute('role', 'alert'), this.classList.add('tk-alert--show'), this.type || this.setAttribute('type', 'info'), (this.hasAttribute('dismiss') || this.hasAttribute('acknowledge') || this.hasAttribute('href') && '' !== this.getAttribute('href')) && !this.querySelector('button.tk-alert--close') && !this.querySelector('button.tk-alert-button--close') && this.appendCloseButton.bind(this)(), this.dispatchCustomEvent('tk.alert.show'), this.closeButton && this.closeButton.focus();
      ...
      switch (a) {case 'type':
          c && -1 !== ['info', 'primary', 'warning', 'success', 'danger'].indexOf(c) || (this.type = 'info');break;case 'dismiss':case 'acknowledge':
          c && 'true' !== c ? this.firstElementChild.tagName && 'button' === this.firstElementChild.tagName.toLowerCase() && this.removeCloseButton.bind(this)() : this.firstElementChild && this.firstElementChild.tagName && 'button' !== this.firstElementChild.tagName.toLowerCase() && this.appendCloseButton.bind(this)();break;case 'href':
          c && '' !== c ? this.firstElementChild.tagName && 'button' !== this.firstElementChild.tagName.toLowerCase() && this.firstElementChild.classList.contains('tk-alert-button--close') && this.appendCloseButton.bind(this)() : this.firstElementChild.tagName && 'button' !== this.firstElementChild.tagName.toLowerCase() && this.removeCloseButton.bind(this)();break;case 'auto-dismiss':
          c && '' !== c || this.removeAttribute('auto-dismiss');break;default:}
    } }, { key: 'buttonCloseFn', value: function buttonCloseFn() {
      this.dispatchCustomEvent('tk.alert.buttonClicked'), this.href && (window.location.href = this.href), this.close();
    } }, { key: 'close', value: function close() {
      var a = this;this.dispatchCustomEvent('tk.alert.close'), this.addEventListener('transitionend', function () {
        a.dispatchCustomEvent('tk.alert.closed'), a.parentNode.removeChild(a);
...
}(HTMLElement);customElements.define('tk-alert', TkAlertElement);
