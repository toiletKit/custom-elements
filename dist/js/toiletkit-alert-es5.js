...
var _createClass = function () {
  ...
  function b() {
    return _classCallCheck(this, b), _possibleConstructorReturn(this, (b.__proto__ || Object.getPrototypeOf(b)).apply(this, arguments));
      ...
      this.dispatchCustomEvent('toiletkit.alert.show'), this.setAttribute('role', 'alert'), this.classList.add('toiletkit-alert--show'), this.type || this.setAttribute('type', 'info'), (this.hasAttribute('dismiss') || this.hasAttribute('acknowledge') || this.hasAttribute('href') && '' !== this.getAttribute('href')) && !this.querySelector('button.toiletkit-alert--close') && !this.querySelector('button.toiletkit-alert-button--close') && this.appendCloseButton.bind(this)(), this.dispatchCustomEvent('toiletkit.alert.show'), this.closeButton && this.closeButton.focus();
      ...
      switch (a) {case 'type':
          ...
          c && '' !== c ? this.firstElementChild.tagName && 'button' !== this.firstElementChild.tagName.toLowerCase() && this.firstElementChild.classList.contains('toiletkit-alert-button--close') && this.appendCloseButton.bind(this)() : this.firstElementChild.tagName && 'button' !== this.firstElementChild.tagName.toLowerCase() && this.removeCloseButton.bind(this)();break;case 'auto-dismiss':
          c && '' !== c || this.removeAttribute('auto-dismiss');break;default:}
    } }, { key: 'buttonCloseFn', value: function buttonCloseFn() {
      this.dispatchCustomEvent('toiletkit.alert.buttonClicked'), this.href && (window.location.href = this.href), this.close();
    } }, { key: 'close', value: function close() {
      var a = this;this.dispatchCustomEvent('toiletkit.alert.close'), this.addEventListener('transitionend', function () {
        a.dispatchCustomEvent('toiletkit.alert.closed'), a.parentNode.removeChild(a);
      }), this.classList.remove('toiletkit-alert--show');
      ...
      if (!(this.querySelector('button.toiletkit-alert--close') || this.querySelector('button.toiletkit-alert-button--close'))) {
        var a = document.createElement('button');if (this.hasAttribute('dismiss') ? (a.classList.add('toiletkit-alert--close'), a.innerHTML = '<span aria-hidden="true">&times;</span>', a.setAttribute('aria-label', this.textClose)) : (a.classList.add('toiletkit-alert-button--close'), a.innerHTML = this.hasAttribute('acknowledge') ? this.textAcknowledge : this.textDismiss), this.closeButton = a, this.firstChild ? this.insertBefore(a, this.firstChild) : this.appendChild(a), a && a.addEventListener('click', this.buttonCloseFn.bind(this)), 0 < this.autoDismiss) {
          var b = this,
              c = this.autoDismiss;setTimeout(function () {
            b.dispatchCustomEvent('toiletkit.alert.buttonClicked'), b.href && (window.location.href = b.href), b.close();
          }, c);
        }
      }
...
}(HTMLElement);customElements.define('toiletkit-alert', JoomlaAlertElement);
